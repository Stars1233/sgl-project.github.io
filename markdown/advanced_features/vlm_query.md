...
```
    ...
    [2026-03-25 16:08:03] INFO utils.py:148: Note: detected 128 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-25 16:08:03] INFO utils.py:151: Note: NumExpr detected 128 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-25 16:08:03] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    2026-03-25 16:08:06.963 DEBUG Persistent cache disabled, using in-memory JIT cache
    ...
    2026-03-25 16:08:06.964 DEBUG Persistent cache disabled, using in-memory JIT cache
    ...
    2026-03-25 16:08:19.125 DEBUG Persistent cache disabled, using in-memory JIT cache
    [2026-03-25 16:08:19] Persistent cache disabled, using in-memory JIT cache
    2026-03-25 16:08:19.125 DEBUG Persistent cache disabled, using in-memory JIT cache
    [2026-03-25 16:08:19] Persistent cache disabled, using in-memory JIT cache
    2026-03-25 16:08:19.125 DEBUG Persistent cache disabled, using in-memory JIT cache
    [2026-03-25 16:08:19] Persistent cache disabled, using in-memory JIT cache
...
```
    ...
    Model response:
    The image shows two yellow taxis parked in an urban street. One of the taxis is equipped with a cleverly designed clothes rack mounted on its roof, allowing someone to hang and dry their clean laundry. This innovative setup can be particularly useful in busy city environments where space is limited.
...
```
    ...
    In the picture, there are two yellow taxis on the street. They appear to be part of a significant recurring feature visible in numerous photos found on social media and were purportedly related to the singer Whitney Houston. The taxis were parked in various locations across the city, including New York City and Glasgow, and the story behind these taxis was a commercial project initiated byーめ even though some people believed it was related to a personal tragedy involving the singer.
...
```
    ...
    The image shows a street scene with a yellow cab and a person wearing a yellow shirt standing on a set of slanted wooden sticks. The person is adjusting or setting up something, possibly related to public art or a performance. The background features tall buildings and multiple flags, suggesting that this might be in an urban area with a lively atmosphere.
