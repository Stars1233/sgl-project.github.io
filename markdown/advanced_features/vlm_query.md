...
```
    ...
    [2026-03-26 03:35:06] INFO utils.py:148: Note: detected 192 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-26 03:35:06] INFO utils.py:151: Note: NumExpr detected 192 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-26 03:35:06] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    2026-03-26 03:35:09.848 DEBUG Persistent cache disabled, using in-memory JIT cache
    ...
    2026-03-26 03:35:20.029 DEBUG Persistent cache disabled, using in-memory JIT cache
    [2026-03-26 03:35:20] Persistent cache disabled, using in-memory JIT cache
    2026-03-26 03:35:20.029 DEBUG Persistent cache disabled, using in-memory JIT cache
    [2026-03-26 03:35:20] Persistent cache disabled, using in-memory JIT cache
    2026-03-26 03:35:20.029 DEBUG Persistent cache disabled, using in-memory JIT cache
    [2026-03-26 03:35:20] Persistent cache disabled, using in-memory JIT cache
...
```
    ...
    Model response:
    The image shows a man cleaning two yellow taxi cabs on a city street. The man is using a scrub brush to scrub off dirt and debris from the backs of two taxis that are parked side by side. The taxis appear to be American-style taxis, known for their distinctive yellow color. The setting suggests an urban environment, possibly in a commercial area.
...
```
    ...
    The image shows a yellow Chicago taxi parked on a city street. The cab is unusual because it has a doll dressed in a blue dress on the back seat. The scene appears to be taken during the day, with no significant people or vehicles nearby. The flag on the cab is not red and white as one might expect from a Chicago taxi, but it is a small flag that is likely to stand for a specific event or organization related to the "Scott Cafe" mentioned in the description.
...
```
    ...
    The image shows a yellow taxi cab driving down a city street. The taxi has a rear-view mirror around which a drawing or artistic work is made, which appears to be a representation of the taxi cab. The artistic setup around the rear-view mirror adds a creative and unique element to an otherwise ordinary taxi scene.
