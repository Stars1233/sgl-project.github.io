...
```
    ...
    [2026-03-22 14:45:48] INFO utils.py:148: Note: detected 192 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-22 14:45:48] INFO utils.py:151: Note: NumExpr detected 192 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-22 14:45:48] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    Model response:
    In the image, there are two yellow taxi cabs on a city street. One taxi is behind the other. The person in the yellow shirt appears to be hanging laundry on a clothesline attached to the back of the rear taxi. The city street has multiple traffic lights, some signs, and a pattern of windows and storefronts.
...
```
    ...
    The image shows a yellow taxi on a city street. The taxi is being pulled by a rack of clothes, suggesting a street vendor or clothing dresser is using it to transport pieces of clothing. The street is lined with tall buildings and flags, indicating it's likely a downtown area of a large city.
...
```
    ...
    The image shows a有人黄色的出租车停在广告牌下。
