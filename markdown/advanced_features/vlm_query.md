...
```
    ...
    [2026-03-24 17:49:13] INFO utils.py:148: Note: detected 192 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-24 17:49:13] INFO utils.py:151: Note: NumExpr detected 192 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-24 17:49:13] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    2026-03-24 17:49:16.995 DEBUG Persistent cache disabled, using in-memory JIT cache
    ...
    2026-03-24 17:49:27.191 DEBUG Persistent cache disabled, using in-memory JIT cache
    [2026-03-24 17:49:27] Persistent cache disabled, using in-memory JIT cache
    2026-03-24 17:49:27.191 DEBUG Persistent cache disabled, using in-memory JIT cache
    [2026-03-24 17:49:27] Persistent cache disabled, using in-memory JIT cache
    2026-03-24 17:49:27.191 DEBUG Persistent cache disabled, using in-memory JIT cache
    [2026-03-24 17:49:27] Persistent cache disabled, using in-memory JIT cache
...
```
    ...
    Model response:
    The image shows two yellow taxi cabs parked on a city street. One cab is positioned behind a piece of advertising or promotional material, which includes an American flag. The scene appears to be in an urban environment, possibly in a busy city like New York, given the styling of the taxis.
...
```
    ...
    The image shows a yellow taxi in New York City, with a shirt draped over the backrest of the driver's seat and hung over the car's rearview mirror. This is one of the traditional and iconic formations that taxi drivers perform, called the 'boot sign,' during their routes to signal to passengers the availability of a ride.
...
```
    ...
    The image shows a yellow taxi in an urban setting, possibly New York City, indicated by the iconic yellow taxis and the surroundings. The taxi is in the foreground, and there is a billboard or some sort of pole with mounted items, possibly related to the driver holding a cloth item on a stand. The background includes other cars and city buildings with signs. This scene might depict a light-hearted or commercial event happening during a taxi ride.
