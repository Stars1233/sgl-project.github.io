...
```
    ...
    [2026-03-26 04:08:25] INFO utils.py:148: Note: detected 192 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-26 04:08:25] INFO utils.py:151: Note: NumExpr detected 192 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-26 04:08:25] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    2026-03-26 04:08:38.272 DEBUG Persistent cache disabled, using in-memory JIT cache
    [2026-03-26 04:08:38] Persistent cache disabled, using in-memory JIT cache
    2026-03-26 04:08:38.272 DEBUG Persistent cache disabled, using in-memory JIT cache
    [2026-03-26 04:08:38] Persistent cache disabled, using in-memory JIT cache
    2026-03-26 04:08:38.272 DEBUG Persistent cache disabled, using in-memory JIT cache
    [2026-03-26 04:08:38] Persistent cache disabled, using in-memory JIT cache
...
```
    ...
    Model response:
    The image shows two yellow taxis driving down a city street. One taxi has aperson dressed in a yellow shirt and blue shorts using a clothespin to hang up a blue piece of clothing on a clothesline attached to the top of the taxi. The clothesline is set up on the side of the rideway or curb, indicating that the person is attempting to dry the clothing in an unusual and unconventional manner. The scene appears to take place in an urban environment, possibly in a busy area, as there are shops and buildings visible in the background. The presence of characters and actions like hanging clothes, or presumably maintaining clothing items, are usually
...
```
    ...
    The image shows a street scene with two yellow taxis parked or moving on a city street. There are also several hanging items, which appear to be laundry, being displayed by someone attached to a backrest bar on the sidewalk. The background includes buildings and street lights, suggesting an urban environment.
...
```
    ...
    The image shows two taxis, one in a standard yellow color and another in a yellow with blue傲业的颜色. The taxi with the blue傲业脫ния is being reflected in the window of a nearby building. The scene takes place in an urban setting, likely a city street.
