...
```
    ...
    [2026-03-24 20:56:14] INFO utils.py:148: Note: detected 192 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-24 20:56:14] INFO utils.py:151: Note: NumExpr detected 192 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-24 20:56:14] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    2026-03-24 20:56:26.999 DEBUG Persistent cache disabled, using in-memory JIT cache
    [2026-03-24 20:56:26] Persistent cache disabled, using in-memory JIT cache
    2026-03-24 20:56:26.999 DEBUG Persistent cache disabled, using in-memory JIT cache
    [2026-03-24 20:56:26] Persistent cache disabled, using in-memory JIT cache
    2026-03-24 20:56:26.999 DEBUG Persistent cache disabled, using in-memory JIT cache
    [2026-03-24 20:56:26] Persistent cache disabled, using in-memory JIT cache
...
```
    ...
    Model response:
    The image shows an unusual and humorous scene where a taxi is parked next to a rear-mounted他们是(mansion liquidation) pole. They sell a variety of goods.
...
```
    ...
    The image shows a street scene in an urban area, likely New York City, given the yellow taxis with the iconic white stripe. The focus is on a person dressed in bright yellow attire, creating a bright contrast against the background. The person appears to be using a stand to display or fold some items. The setting is outdoors, and while the background shows a mix of stores and billboards, the primary subject is the person in the yellow top. The street is relatively busy, with other vehicles and people visible in the distance.
...
```
    ...
    The image shows a street scene in an urban area, possibly in New York City, given the recognizable yellow taxis. There is a person standing next to a yellow taxi that has opened its back door, revealing a clothes line with various pieces of clothing hanging on it. The person appears to be in the middle of adjusting or moving the clothes. The background includes buildings, likely shops or offices, with several shop signs. There are also several signs in the background, likely indicating exits or directions, typical of streets in densely populated urban areas.
