...
```
    ...
    [2026-03-25 11:09:02] INFO utils.py:148: Note: detected 192 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-25 11:09:02] INFO utils.py:151: Note: NumExpr detected 192 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-25 11:09:02] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    2026-03-25 11:09:09.636 DEBUG Persistent cache disabled, using in-memory JIT cache
    ...
    2026-03-25 11:09:18.700 DEBUG Persistent cache disabled, using in-memory JIT cache
...
```
    ...
    Model response:
    The picture shows a scene in a street where a taxicab has been modified to have its windows removed and isMerTEXT shown via an ironing board with clothes draped over the back, creating the appearance of an ironer.
...
```
    ...
    It looks like the image shows two yellow taxi cabs parked on a city street. One taxi is closer to the foreground and appears to be more stationary, while the other is positioned further back near street lamps. The background includes various urban elements such as buildings, street signs, and possibly pedestrians. The overall scene gives the impression of a busy city environment.
...
```
    ...
    The image you've provided is of a yellow taxi from the movie "The Hangover." In the scene, a man dressed in a suit and holding a tie is being fed by people who are filming the film. The出租车停在街上, leaning against a pole, and there are several people standing around it.
