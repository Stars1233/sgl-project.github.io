...
```
    ...
    [2026-03-24 20:20:59] INFO utils.py:148: Note: detected 192 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-24 20:20:59] INFO utils.py:151: Note: NumExpr detected 192 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-24 20:20:59] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    2026-03-24 20:21:02.340 DEBUG Persistent cache disabled, using in-memory JIT cache
    ...
    2026-03-24 20:21:12.683 DEBUG Persistent cache disabled, using in-memory JIT cache
    [2026-03-24 20:21:12] Persistent cache disabled, using in-memory JIT cache
    2026-03-24 20:21:12.683 DEBUG Persistent cache disabled, using in-memory JIT cache
    [2026-03-24 20:21:12] Persistent cache disabled, using in-memory JIT cache
    2026-03-24 20:21:12.683 DEBUG Persistent cache disabled, using in-memory JIT cache
    [2026-03-24 20:21:12] Persistent cache disabled, using in-memory JIT cache
    2026-03-24 20:21:12.684 DEBUG Persistent cache disabled, using in-memory JIT cache
    [2026-03-24 20:21:12] Persistent cache disabled, using in-memory JIT cache
    2026-03-24 20:21:12.684 DEBUG Persistent cache disabled, using in-memory JIT cache
    [2026-03-24 20:21:12] Persistent cache disabled, using in-memory JIT cache
...
```
    ...
    Model response:
    The person in the image is hanging clothes on a clothesline attached to a yellow taxi. The taxi is parked on a city street with buildings and pedestrians visible in the background. The clothes consist of colorful items, likely representing忂doing laundry in an unconventional urban setting.
...
```
    ...
    The image shows a scene of dressage training for a horse. The bar in the middle is typically used to support the reins when controlling a horse during dressage. In dressage competitions, riders guide their horses by manipulating the reins and using the barrier to keep their horse rounded and "in the bend," as this helps the horse maintain a straight line and creates the desired movement. The covering over the共和国障碍is likely to be something that the rider or horse may use during training, possibly to provide additional support or to keep loose parts of the blanket out of the way.
...
```
    ...
    The image shows a person operating a fake taxicab, which customers can board without using their real cab. The person is wearing a yellow shirt and is holding the taxi from behind, simulating being a customer boarding. The fake taxi is small and yellow, likely a plaything or a part of a performance or event. The background includes other yellow taxis and some buildings with flag banners.
